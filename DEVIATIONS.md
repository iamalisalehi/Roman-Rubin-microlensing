...
## 50. Step S1: a sample-event dump, so single events can be drawn at all (2026-09-21)

**What the plan said.** Nothing. This is new, requested for an illustrative figure set in the
style of Sajadian & Makler (arXiv:2608.16448): per-band light curves and astrometric tracks,
each with and without parallax, with instrument error bars, for both telescopes, across a
spread of physics and detection classes. It is a new step series (S1 dump, S2 plotter, S3 short
runs), not a plan step.

**Why the finished runs cannot supply it.** The per-event table is one row per event. The
per-epoch data -- `l->timn/magn/errm/soux/souy/erra/tele` -- lives only inside the time loop,
goes to `FisherM`, and is overwritten by the next draw. Worse for this purpose, `l->magn[]` holds
only Rubin's *representative band* (`magniRubinRef`, r by default), so even a dump of those
arrays would not give the six-band light curve. And the table lacks `mus1, mus2, mul1, mul2`,
the source and lens proper-motion components, so the absolute sky tracks cannot be rebuilt from
it either. A specific production event cannot be cheaply regenerated: the RNG stream is
sequential across sightlines (why the H3 run matched 0 of 2,673 v3 events).

**A legacy dump existed and is dead code.** `fil4 -> magC0.dat` and `fil5 -> datC0.dat` write
nearly the right quantities, gated on `flagm`, which is set only inside `if (... save < 0 ...)`.
`save` is initialised to 0 and its only increment is inside that same block, so the gate can
never open. Both files are 0 bytes in the repo and in `runs/prod_bh_20260917/`. It was left
alone rather than revived, because even revived it is Earth-frame only (`lightcurve(..., 0)`),
Rubin-epochs only (it sits in the LSST branch), one event, `legacyId == 1` (bulge) only -- and it
**calls `RandN()`**, so switching it on would change which events a run simulates. Recorded in
`OPEN_ITEMS.md` rather than deleted.

**What was done.** `--dump-samples <spec>`, off by default. During the time loop, both observing
branches push one `DumpEpoch` per recorded visit into an in-memory buffer: time, telescope tag,
**that visit's filter**, the noisy measurement the detection test drew, the model magnitude with
and without microlensing parallax, the instrument's photometric error, `u` and `u` without
parallax, both magnifications, the centroid deflection with and without parallax, the unlensed
source track, the lensed centroid, the lens track, and the instrument's astrometric error. After
`FisherM`, the event is tested against the spec's classes and, if it fills one, written as
`<class>_<id>_{epochs,model,params}.dat`. The model file is a dense curve regenerated by calling
the real `lightcurve()` for BOTH `tele = 0` and `tele = 1` on a two-tier grid (a peak window
sampled in units of the event's own `tE`, plus 2-day steps over the decade for the parallax
wobble), in all seven filters.

**Four design decisions worth seeing.**
1. **Zero RNG consumed.** Every dumped value was already computed; `mag_obs` is the existing
   `magnio`/`magnioR`. This is the property the legacy dump lacked, and it is tested below.
2. **Buffer then commit**, because the detection class is unknown until the loop ends.
3. **Per-epoch magnitude is the observed filter only.** At a Roman epoch, `magni0[0..5]` still
   hold *Rubin-frame* values from earlier in the timestep; dumping all seven there would write
   six stale numbers. The model file supplies all seven, recomputed per frame.
4. **No 2D noisy astrometric datum exists in this simulation.** The astrometric chi-square is
   built on the scalar `trajp = |pos_c|` with one scalar noise draw. The dump therefore gives the
   model track plus a genuine per-epoch `err_ast`; any 2D scatter drawn in a figure must be
   generated in the plotter and labelled illustrative. Manufacturing it in C++ would consume RNG.

**`_params.dat` is a key/value block, not the event's table row.** The brief proposed the row;
emitting it would have meant converting the 40-line `filg_in <<` block to an `ostringstream`,
re-indenting every continuation line for no scientific gain. The key/value block also carries
`mus1/mus2/mul1/mul2`, which the table never has.

**A defect in the selector, found by the acceptance run.** All three `gap_filler` events it wrote
had `ndw_R = 0`: Roman never observed those sightlines. `t0zone` is computed from the
mission-wide season schedule and knows nothing about pointing, so it reads 1 ("mid-mission gap")
for a sightline outside the GBTDS footprint exactly as for one inside it. That is the
out-of-footprint case the class exists to exclude -- Rubin observing where Roman does not look is
not Rubin filling a gap in Roman's coverage. **Fix (approved and applied):** require
`ndw_R > 0`; and apply the same test (plus `ndw_L > 0`) to the four "physics" classes (`any`,
`ns_typical`, `bh_short`, `bh_long`), which are drawn to show what both telescopes see and would
otherwise be supplied mostly from the 97% of the scan area Roman does not cover.

**`rubin_only` filled 0 of 2 in the acceptance run, and that is physics, not a bug.** It requires
Roman epochs within +-2 tE of the peak AND no Roman detection. At the stub's footprint sightline
Roman has ~50,000 epochs at 12.1-minute cadence and detects essentially everything Rubin does. The
class needs footprint sightlines with thinner or season-edge coverage, which is a requirement on
Step S3's sightline choice.

**Verification.**
- `g++ -fsyntax-only` clean; `make` gives the six pre-existing unused-variable warnings, none new.
  The patch deletes no existing line.
- **Byte-identical acceptance test** -- two isolated run directories, identical flags
  (`--stub --events 10 --lenses 2 --nerr 0.05 --maxdraws 500`), one with `--dump-samples`:
  36 sightlines, 543 events each. `test5.dat`, `MapLMC5.dat`, `EfLMC5.dat`, `EfLMC5B.dat`,
  `LpLMC5.dat` all **identical** (SHA-256 `59e531ce...`, `e6427ab1...`, `1218fa4d...`,
  `7b4c6b0b...`, `93ae4650...`). `run_provenance.txt` differs in exactly the one
  `dump_samples` line.
- `./fishertest` PASSES.
- Content: all six Rubin filters present in a `both` event (u 22, g 540, r 545, i 714, z 521,
  y 38 epochs); Roman epochs 0.0084 d = 12.1 min apart in F146; the model magnitude at `t = 2 d`
  equals `magb[z]` to the digit.

**Cost.** Nothing measurable with the dump off (one branch per epoch). With it on, one
`push_back` per epoch, the vector keeping its capacity across draws. **Disk is larger than the
brief estimated:** the brief's ~1 MB per event was the model file (1.4-1.5 MB, as predicted); the
