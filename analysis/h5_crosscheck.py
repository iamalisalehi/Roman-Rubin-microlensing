...
with this header prepended:
    ...
    condB_L condB_R w_area nepL_pk nepR_pk
...
ROMAN_AST_FLOOR = 1.1   # mas, Step H4: 1% of the 110 mas pixel
...
def report_signal(cov):
    ...
    print(f"  fraction with u0 <= sqrt(2), i.e. reaching the astrometric peak: "
          f"{(cov.u0 <= r2).mean():.1%}")
    print(f"\n  Roman per-exposure astrometric precision: {ROMAN_AST_FLOOR} mas")
    print(f"  median peak signal / single-exposure precision: "
          f"{cov.shift_max.median()/ROMAN_AST_FLOOR:.4f}")
    print(f"  Roman exposures per event (ndwR): median {int(cov.ndwR.median()):,}")
    print(f"  an INDEPENDENT floor would average to "
          f"{ROMAN_AST_FLOOR/np.sqrt(cov.ndwR.median()):.4f} mas over that many")
