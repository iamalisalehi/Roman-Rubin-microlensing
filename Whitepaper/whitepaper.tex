...
\begin{document}
	...
	\textbf{The signal is far below a single exposure.} The median maximum shift is $0.120$~mas; $81.0\%$ of events have a trajectory that reaches $u=\sqrt{2}$ and therefore attain the full $\theta_E/\sqrt{8}$. Roman's per-exposure astrometric precision \emph{for these sources} is $6.96$~mas at the median -- not the $1.1$~mas bright-source floor, which does not describe a faint bulge main-sequence star. Only $0.020\%$ of events have a peak shift exceeding a single exposure's precision. (Event-rate weighted, Section~\ref{sec:weighting}; the raw sample gives $0.111$~mas, $74.1\%$ and $0.11\%$. The weight favours short, fast events, which have smaller $\theta_E$ but pass through $u=\sqrt{2}$ more often.)

	The measurement therefore exists entirely through averaging. With a median of $\sim$$5\times10^{4}$ exposures per detected event, an error independent between exposures averages to $0.031$~mas, against which the median $0.120$~mas signal stands at $3.9\sigma$. \textbf{That factor of $\sim$$224$ is the whole astrometric result, and that it is available is an assumption, not a measurement} -- see Section~\ref{sec:openitems}. Every astrometric number in this paper should be read with the clause \emph{if the per-exposure astrometric error is independent between exposures} attached to it.
	...
	\begin{itemize}
		...
		\item \textbf{The satellite-parallax numbers are not event-rate weighted, and cannot be from the data that exist.} Everything else here carries Section~\ref{sec:weighting}'s weight, including the astrometric-shift section. The paired satellite-parallax run predates the columns the weight needs: $W\propto\sqrt{M_L}\,v_t\,Z(D_s)$, and a paired row records $\theta_E$ and $\pi_E$ but neither distance, so $v_t$ and $Z$ cannot be reconstructed ($\pi_{\rm rel}=1/D_l-1/D_s$ is one equation in two unknowns). The simulator now writes $M_L$, $D_l$, $D_s$ and $v_t$ into that file, so the next paired run is weightable; until then the $0.9924$ median ratio and the $36.7\%$ improving by more than $1\%$ are raw-sample statistics. The pairing itself -- the same event evaluated twice -- carries no weight and is unaffected, as are the sign test and the bit-exact control.
