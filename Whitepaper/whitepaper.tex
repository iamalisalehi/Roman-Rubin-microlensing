...
\begin{document}
	...
	The lens mass function is Kroupa (2001) \cite{Kroupa2001} plus stellar remnants, drawn over a bulge-appropriate mass range. This replaces a mass range of $3$--$5000\,M_\odot$ inherited unchanged from an LMC simulation, which is a MACHO dark-matter search range and not a bulge population at all. The consequences of that inheritance were not subtle: the first complete production run returned a median lens mass of $386.9\,M_\odot$, a median Einstein crossing time of $953$~d, a median angular Einstein radius of $13.75$~mas and a median parallax of $0.005$, where the bulge's median lens is a few tenths of a solar mass and a typical event lasts tens of days. \textbf{No number in any earlier draft of this document that predates this correction describes a Galactic bulge}, and such numbers should be assumed wrong rather than merely imprecise.
	...
	Two features of that test deserve stating explicitly, because both were changed after earlier versions of this work and no result predating the change survives it. First, the statistic is \emph{signed}. It is the improvement a lensing model achieves over a flat baseline, and taking its absolute value -- as was previously done -- would let an event whose fitted model is \emph{worse} than the baseline pass the bar on the strength of how badly it fails.

	Second, the threshold is a fixed number and not, as previously, twice the epoch count. $\Delta\chi^2$ here is a difference between two \emph{nested} models, so under the null hypothesis it is distributed as $\chi^2$ with $p$ degrees of freedom, where $p$ is the number of extra parameters the lensing model carries -- a small, fixed number with mean $p$ and variance $2p$. It is not distributed with $n_{\mathrm{dw}}$ degrees of freedom, and a bar proportional to $n_{\mathrm{dw}}$ therefore does not hold significance constant: it thresholds the \emph{mean per-epoch} improvement instead, and so demands more total evidence from an event simply because more data were taken on it. Inside Roman's footprint, where $n_{\mathrm{dw}}\approx5\times10^{4}$, that bar reached $\sim10^{5}$, far above any physically meaningful significance. The fixed value of $500$ follows \cite{penny2019predictions}, so that yields here are conditioned on the same criterion as the reference Roman forecast; it is recorded in each run's provenance block and is adjustable at the command line, and every yield quoted in this paper is conditioned on it.
	...
	Each derivative is estimated from two perturbed evaluations of the model, averaged. For every parameter except the two blend fractions the steps are equal and opposite, which is algebraically equivalent to a standard central difference; the blend fractions use a step selected from the value of $f_b$ itself, so that the perturbation cannot carry it outside its physical range $[0,1]$. An earlier version of this code differenced $t_E$ and $\pi_E$ with both evaluations on the same (positive) side of the parameter value, apparently to avoid evaluating the model at an unphysical negative value. That reasoning has since been checked: the constraint is not binding at the step sizes actually used, while the one-sided scheme is first-order and biased where the symmetric form is second-order. Both now use the symmetric form. The choice of step size is not free either, and is established by an explicit convergence test in Appendix~\ref{app:stepsize}.
	...
	The normalised matrix is then inverted using GSL's LU decomposition \cite{GalassiGSL2009} (\texttt{invert\_matrix}). An earlier version of this code perturbed the diagonal by $10^{-10}$ when the matrix was found to be exactly singular and inverted it regardless; this is no longer done, because a singular Fisher matrix is not a numerical obstacle to be worked around but the finding that the data cannot constrain some combination of parameters. Each event's condition number $\lambda_{\max}/\lambda_{\min}$ is computed from the normalised matrix and recorded, and a partition whose matrix is singular, is above a threshold condition number, or has fewer epochs than free parameters is reported as \emph{not characterisable} rather than being assigned an uncertainty. Appendix~\ref{app:stepsize_eigen} discusses what these degenerate directions correspond to physically. Formally, the Fisher matrix's inverse is exactly the Cram\'er-Rao bound on achievable parameter precision \cite{VakiliHogg2016} referenced in Section~\ref{sec:error_models} -- so this step is the multi-parameter generalization of the same information-theoretic limit that sets the single-parameter centroiding precision discussed there.
	...
	\section{Results as they stand}
	\label{sec:results}

	This section states what the simulation currently says. It is written for collaborators rather than referees: where a result is provisional, or rests on a component that is still a placeholder, that is said here rather than left for the reader to discover.

	\subsection{What combining the two surveys buys}

	The headline result of the joint analysis is a \textbf{yield} effect and not a precision effect, and that distinction is the more defensible claim as well as the one the data support. Combining the two surveys characterises $245$ events that neither survey characterises alone -- events whose single-survey Fisher matrices are rank-deficient but whose joint matrix inverts. These are exactly the population that a naive ratio of per-survey precisions would discard, since no ratio is defined for them.

	Where both surveys \emph{can} characterise an event on their own, the precision gain from combining is modest. The gap-filling comparison came out in the \emph{reverse} of the ordering originally predicted: the largest fractional improvements are on short events peaking inside a Roman season gap, where Rubin supplies the only data there is, and not on long events where a longer baseline was expected to matter. Per-event median ratios of $\sigma_{\pi_E}(\mathrm{joint})/\sigma_{\pi_E}(\mathrm{Roman\ alone})$ for events peaking in a gap are $0.31$, $0.80$, $0.95$ and $0.984$ in $t_E$ bins of $10$--$30$, $30$--$100$, $100$--$300$ and $>300$~d. For long events Roman alone already measures the parallax at $\sim5\%$, so there is little left for Rubin to add.

	Gap-filling also buys less on the parallax than on the timescale -- $0.31$ against $0.13$ in the short bin -- which is physically sensible: Rubin's cadence resolves the rise and fall that set $t_E$, whereas $\pi_E$ is a subtler distortion of the light-curve shape and is harder to recover from sparser ground-based photometry.

	\subsection{The astrometric signal}

	Astrometric microlensing is \textbf{not} a per-epoch detection in the GBTDS. The median maximum centroid shift is $0.121$~mas against a median Roman per-exposure astrometric precision of $5.28$~mas at these (faint) source magnitudes, and only $0.1\%$ of events have a shift exceeding Roman's own single-exposure precision. $\theta_E$ is nonetheless well forecast, because the survey stacks: the constraint comes from many epochs of a coherent trajectory rather than from any single measurement resolving the shift.

	\subsection{How well anything is measured at all}

	Every comparison above is a \emph{ratio}, and a ratio is unfalsifiable without the absolute scale it is a ratio of: a factor of two improvement on a $300\%$ error is not a measurement. The forecasts are therefore also reported directly, as cumulative distributions of the fractional $1\sigma$ forecast on $t_E$ and $\pi_E$ from the photometric matrix, on $\theta_E$ from the astrometric matrix, and on the derived lens mass $M_L = \theta_E/(\kappa\pi_E)$, for each of the three partitions separately. Two further panels carry the diagnostics that decide how much of it to believe: the per-event joint-against-single scatter, which must satisfy $\sigma_{\mathrm{joint}} \le \sigma_{\mathrm{single}}$ event by event, and the distribution of condition numbers, which says which inverses are trustworthy.

	Those distributions are normalised to the full detected sample rather than to the measured subset, deliberately. Normalising to the subset that happens to invert would report the precision of the events that were easy to characterise and silently drop the rest, which is the same selection error that makes a naive ratio misleading. Alongside these, the $(t_E,\pi_E)$ plane is mapped to show \emph{where} in parameter space the combination characterises events that a single survey cannot -- which is the same population as the $245$-event yield gain above, shown as a region rather than a count.

	\subsection{Satellite parallax: the separation is small, and its effect is not yet measured}
	\label{subsec:satpar}

	Roman observes from L2 and Rubin from the ground, so the two see the source along slightly different lines of sight and the microlens parallax acquires a \emph{simultaneous} baseline in addition to the time-evolving annual one. The observable is the observer separation projected into Einstein radii,
	\[
	\Delta u_{\mathrm{sat}} = \pi_E\,D_\perp/\mathrm{AU},
	\]
	recorded per event. Over joint detections with Roman coverage near the peak its median is $\mathbf{0.0022}\,\theta_E$, its $95$th percentile $0.0096$, and its maximum $0.039$. The two observers are separated by a few thousandths of an Einstein radius, so the light-curve perturbation is small and a large gain in parallax precision would be surprising on physical grounds.

	\textbf{What that separation buys in forecast precision is not yet measured, and no number for it is quoted here.} The natural experiment -- two runs differing only in the offset, matched event by event -- is impossible in this code, and for a physical reason rather than a bookkeeping one: moving the observer changes which events are detectable, the random-number stream is shared across the scan, and the two runs consequently diverge from the first sightline where the effect is non-zero. Measured, they agree exactly for $1{,}740{,}091$ events and then part company at precisely the first field inside Roman's footprint. An unpaired comparison of the two populations was attempted and rejected by its own control: it reported the satellite making the \emph{timescale} forecast $15\%$ worse, which is impossible for matched events and instead measures how much the two detected samples differ.

	The comparison was therefore re-implemented as a controlled one, evaluating both observer positions for the same event inside a single run. That machinery is verified -- for events with no Roman epochs near the peak, where moving Roman cannot matter, the two forecasts agree to a median ratio of $1.000000$ over $29{,}171$ events -- but for events Roman does cover it fails two consistency checks that follow from the physics: the apparent improvement \emph{shrinks} as the observer separation grows, and $\sigma_{t_E}$ degrades systematically. Neither is possible for two descriptions of one event, so the ratios are being driven by something other than parallax information, and the cause has not yet been identified. The analysis code withholds the numbers rather than presenting them.

	\subsection{The detection criterion, and what it invalidates}

	The detection bar described in Section~\ref{subsec:detclass} -- a fixed $\Delta\chi^2>500$ on a signed statistic -- replaced a bar proportional to the epoch count. The change roughly doubles the detection rate inside Roman's footprint and raises it by about half outside, and it eliminates a class of inconsistency in which a single survey detected an event that the joint test rejected. \textbf{Every yield in this project that predates this change was conditioned on a different criterion and none of those numbers survive it.} Yields quoted here are conditioned on $\Delta\chi^2>500$, recorded in each run's provenance block.

	\section{Open items}
	\label{sec:openitems}

	Stated plainly, because this document is for people deciding whether to work on this.

	\begin{itemize}
		\item \textbf{Roman's astrometric error model is a placeholder.} Several filter-dependent constants carry either no F146 entry or the original K-band placeholder, and Roman's own astrometric precision curve is not yet a Roman curve. Every astrometric parameter uncertainty in this paper inherits that.
		\item \textbf{Satellite parallax was absent from every result predating the observer-position work.} Results computed before Roman was given a position at L2 implicitly placed it at Earth. Anything involving $\pi_E$ from that era should be regarded as superseded.
		\item \textbf{The precision gain from satellite parallax is unmeasured} (Section~\ref{subsec:satpar}). The paired comparison fails its own physical checks and the reason is not yet known; two untested candidates are that the per-epoch photometric weights are not recomputed when the observer is moved, and that the zero-offset configuration leaves a near-degeneracy that passes the conditioning gate while leaving marginalised errors unstable.
		\item \textbf{Roman's halo orbit is not modelled.} The mean L2 offset is used; the halo orbit adds a periodic wobble on an effect that is already of order $10^{-3}$.
		\item \textbf{The per-sightline stopping criterion is weak in its third floor}, requiring only two well-conditioned Fisher matrices, so a field can complete with very few characterisable events (Section~\ref{sec:fisher_caveats}).
		\item \textbf{The GBTDS footprint is Penny et al.'s, and the footprint has since changed.} Rubin's wide field sees more of the bulge than the adopted region.
		\item \textbf{The Rubin-alone branch has not been validated against an independent published forecast.} Any such comparison must reweight to the other work's sampling first, or the difference in selection will look like a bug in this code.
	\end{itemize}
