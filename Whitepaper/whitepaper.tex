...
\begin{document}
	...
	\begin{tabular}{lcccc}
		...
		\midrule
		$\sigma(t_E)$, peak in a gap    & $\mathbf{0.250}$ & $0.924$ & $0.975$ & $0.990$ \\
		$\sigma(\pi_E)$, peak in a gap  & $\mathbf{0.433}$ & $0.936$ & $0.978$ & $0.991$ \\
		\addlinespace
		$\sigma(t_E)$, peak in a season & $0.984$ & $0.975$ & $0.977$ & $0.992$ \\
		$\sigma(\pi_E)$, peak in a season & $0.986$ & $0.978$ & $0.980$ & $0.992$ \\
		\bottomrule
	\end{tabular}
	\end{center}

	\noindent with $818$, $1{,}148$, $635$ and $193$ in-gap events per bin. The in-season rows are the control, and they are what make the in-gap rows a measurement rather than an offset: where Roman has the event in-season, Rubin sharpens it by about $2\%$ in every $t_E$ bin, and that $2\%$ is the floor against which the short-$t_E$ in-gap number of $0.25$ should be read. Deeper into the gap the effect is larger still: for $10$--$30$~d events peaking more than $45$ days past a season edge, the median ratio is $0.017$ in $t_E$ and $0.063$ in $\pi_E$ -- Roman has essentially nothing and the joint forecast is Rubin's.
	...
	Every comparison above is a ratio, and a ratio is unfalsifiable without the scale it is a ratio of: a factor of two on a $300\%$ error is not a measurement. Figure~\ref{fig:f4} gives the forecasts themselves, as cumulative distributions over the $8{,}894$ Roman-observed detections.
	...
	\textbf{$\theta_E$ is Roman's measurement.} Median fractional precision $\sigma(\theta_E)/\theta_E$ is $0.252$ for the joint fit, $0.269$ for Roman alone and $1.93$ for Rubin alone: ground-based astrometry is not a capability here. Rubin's contribution is a $1.7\%$ tightening of Roman's number -- the paired, event-by-event median of $\sigma_{\mathrm{joint}}/\sigma_{\mathrm{Roman}}$ is $0.983$. (Comparing the two medians instead gives $0.939$, an apparent $6\%$ gain; the ratio of medians is not the median of ratios, and for a paired quantity the per-event statistic is the correct one.) Any claim that the joint fit measures the Einstein radius should attribute it to Roman.

	\textbf{The lens mass is the quantity that genuinely needs both telescopes.} $M_L=\theta_E/(\kappa\pi_E)$ takes $\theta_E$ from Roman's astrometry and $\pi_E$ from the photometric baseline, so it is the one place where a change in either shows up as a change in the science. Counting Roman-observed detections whose mass the joint fit measures to a given precision and \emph{neither survey alone} does: $608$ events at better than $100\%$, $338$ at better than $30\%$, $145$ at better than $10\%$.

	That count is deliberately taken at a precision threshold. Counting instead the events where a mass merely exists -- where the inversion succeeded and the value is not a sentinel -- returns zero events exclusive to the joint fit, because Rubin's matrix inverts for almost all of them with a median fractional error of $15$, i.e. $1500\%$. An inverted matrix is not a measured mass.
